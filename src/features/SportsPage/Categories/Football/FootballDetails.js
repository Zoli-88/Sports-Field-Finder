...
export function FootballDetails() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3005/football/${footBallId}`)
      .then((res) => res.json())
      .then((data) => setFootBallDetails(data));
  }, [footBallId]);
  ...
  async function addToFavorites(e) {
    ...
    await fetch(`http://localhost:3005/favorites`, {
      ...
      body: JSON.stringify({
        userId: user.id,
        name: footballDetails.name,
        images: footballDetails.images,
        phone: footballDetails.phone,
      }),
    }).then((res) => res.json());
  ...
  return (
    <>
      ...
      <header className="detailsHeaderContainer">
        <h1 className="detailsTitle">{footballDetails.name}</h1>
      ...
      <div className="detailsWrapper">
        <div className="detailsContainer">
          <div>
            <img
              src={footballDetails.images}
              alt={`Poster for ${footballDetails.name}`}
            ></img>
          </div>

          <div>
            <Map
              latitude={footballDetails.latitude}
              longitude={footballDetails.longitude}
              name={footballDetails.name}
            ></Map>
          </div>
